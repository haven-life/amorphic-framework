module.exports = function (PersistObjectTemplate) {
    const moduleName = 'Persistor Mongo Update';
    ...
    PersistObjectTemplate.persistSaveMongo = function(obj, promises, masterId, idMap, txn, logger) {
        const functionName = 'persistSaveMongo';
        if (!obj.__template__)
        ...
        if (!schema.documentOf && !masterId) {
            ...
            (logger || this.logger).debug({
                ...
                category: 'milestone',
                message: 'Search for top of ' + obj.__template__.__name__,
                data: {
                    activity: 'save'
                }
            });
            ...
            (logger || this.logger).debug({
                ...
                function: functionName,
                category: 'milestone',
                message: 'Found top as ' + obj.__template__.__name__,
                data: {
                    activity: 'processing'
                }
            });
        ...
        if (idMap[id.toString()]) {
            (logger || this.logger).debug({
                ...
                function: functionName,
                category: 'milestone',
                message: 'Duplicate processing of ' + obj.__template__.__name__ + ':' + id.toString(),
                data: {
                    activity: 'processing'
                }
            });
        ...
        (logger || this.logger).debug({
            ...
            function: functionName,
            category: 'milestone',
            message: 'Saving ' + obj.__template__.__name__ + ':' + id.toString() + ' master_id=' + masterId,
            data: {
                activity: 'save'
            }
        });
                        ...
                        for (ix = 0; ix < value.length; ++ix)
                        {
                            ...
                            if (isCrossDocRef) {
                                ...
                                (logger || this.logger).debug({
                                    ...
                                    function: functionName,
                                    category: 'milestone',
                                    message: 'Treating ' + prop + ' as cross-document sub-document',
                                    data: {
                                        activity: 'processing'
                                    }
                                });
                                ...
                                if (!value[ix][foreignKey] || value[ix][foreignKey].toString() != id.toString()) {
                                    ...
                                    (logger || this.logger).debug({
                                        ...
                                        function: functionName,
                                        category: 'milestone',
                                        nessage: 'updated it\'s foreign key',
                                        data: {
                                            activity: 'processing'
                                        }
                                    });
                                ...
                                {   // This will resolve the id and it won't be a function anymore
                                    (logger || this.logger).debug({
                                        ...
                                        function: functionName,
                                        category: 'milestone',
                                        message: prop + ' waiting for placement, ebmed as subdocument',
                                        data: {
                                            activity: 'processing'
                                        }
                                    });
                                ...
                                (logger || this.logger).debug({
                                    ...
                                    function: functionName,
                                    category: 'milestone',
                                    message: 'Saving subdocument ' + prop,
                                    data: {
                                        activity: 'processing'
                                    }
                                });
                        ...
                        for (ix = 0; ix < value.length; ++ix)
                        {
                            ...
                            if (value[ix].__dirty__) {
                                (logger || this.logger).debug({
                                    ...
                                    function: functionName,
                                    category: 'milestone',
                                    message: 'Saving ' + prop + ' as document because we updated it\'s foreign key',
                                    data: {
                                        activity: 'processing'
                                    }
                                });
