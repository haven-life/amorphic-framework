...
import { getTemplate, recordStatics } from './getTemplate.js'
import flattenTemplates from './flattenTemplates.js'
import UsesV2ReturnPass1 from './UsesV2ReturnPass1.js'
 ...
 */
export async function getTemplates(persistObjectTemplate, appPath, templates, config, appName) {
    ...
    for (let ix = 0; ix < templates.length; ++ix) {
        await getTemplate(templates[ix], {}, templateMode === 'auto',
            ignoringClient,
            requiredTemplates,
            referencedTemplates,
            mixinGraph,
    ...
    if (config && config.appConfig && config.appConfig.modules) {
        for (let moduleKey in config.appConfig.modules) {
            ...
            if (!currentModule.require) {
                ...
                const mixins = (await import(currentModule.require))[moduleKey + '_mixins'];
                if (typeof mixins !== 'function') {
                    ...
                    await addModuleToSource(moduleKey, currentModule);
                }
            }
        }
    }
    ...
    if (ast && !applicationSource[appName] && !(config.appConfig.serverMode === 'daemon' || config.appConfig.serverMode === 'api')) {
        ...
        applicationSource[appName] = result.code;
        applicationSourceMap[appName] = result.map;
    }
    ...
    async function addModuleToSource(moduleKey, module) {
        let moduleExports = await import(module.require);
        let moduleSource;
