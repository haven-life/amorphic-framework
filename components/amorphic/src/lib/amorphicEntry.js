...
import { default as amorphicContext } from './AmorphicContext.js'
import { logMessage } from './utils/logger.js'
import { establishServerSession } from './session/establishServerSession.js'
import { displayPerformance } from './utils/displayPerformance.js'
import { StatsdHelper as statsdUtils } from '@haventech/supertype';

/*
    Set up amorphic for the first time
 */
export async function amorphicEntry(sessions, controllers, nonObjTemplatelogLevel, req, resp, next) {
    ...
    if (req.originalUrl.match(/([A-Za-z0-9_-]*)\.cached.js.map/)) {
        ...
        return establishServerSession(req, appName, 'initial', false, null, sessions, controllers, nonObjTemplatelogLevel)
