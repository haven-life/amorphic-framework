...
import { logMessage as logMessage } from './utils/logger.js'
import { readFile as readFile } from './utils/readFile.js'
import { getTemplates as getTemplates } from './getTemplates.js'
...
export function loadSchema(appSchemaPath, commonSchemaPath, externalSchemas) {
    let schema = null;
    if (externalSchemas) {
...
export function setUpInjectObjectTemplate(appName, config, schema) {
    ...
    if (dbConfig.dbName && dbConfig.dbPath) {
        if (dbConfig.dbDriver === 'mongo') {
            ...
            connectToDbIfNeedBe = Promise.resolve(localKnex); // require('knex') is a synchronous call that already connects
        }
    }
 ...
 */
async function buildAppConfigAndLoadTemplates(appName, config, controllerJsDir, commonJsDir, sessionStore,
 ...
 */
async function loadTemplates(appName) {
    ...
    const templates = await getTemplates(baseTemplate, appConfig.appPath, [prop + '.js'], appConfig, appName);
    return [
        baseTemplate,
        templates
    ];
...
async function loadTSTemplates(appName) {
    ...
    AmorphicContext.applicationTSController[appName] = await (await import(nodePath.normalize(appConfig.appPath + '/' + controllerPath))).Controller;
