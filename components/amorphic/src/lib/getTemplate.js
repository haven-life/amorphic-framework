...
export async function getTemplate(file, options, uses,
    ...
    async function _getTemplate(file, options, uses) {
        ...
        if (options.app) {
            ...
            if (fs.existsSync(daemonPath)) {
                ...
                fileExports = await import(interactivePath);
                requirePath = interactivePath;
            }
        }
        else if (fs.existsSync(appPath + file)) {
            clientPath = appName;
            fileExports = await import(appPath + file);
            requirePath = appPath + file;
        }
        else {
            clientPath = 'common';
            fileExports = await import(config.commonPath + file);
        ...
        if (config.appConfig && config.appConfig.templateMode === 'auto') {
            ...
            let initializerReturnValues = await fileInitializer(persistObjectTemplate, usesV2Pass1);

            persistObjectTemplate.create = oldCreate;
            fileInitializers[fileNoExt] = await fileInitializer;
            ...
            templates = requiredTemplates[fileNoExt] || (await fileInitializer(persistObjectTemplate, getTemplateInject, usesV1));
        ...
        async function usesV1(file, options) {
            return _getTemplate(
                file,
                options,
                true
        ...
        async function getTemplateInject(injectFile, injectOptions, injectUses) {
            return _getTemplate(
                injectFile,
                injectOptions,
                injectUses
        ...
        async function usesV2Pass1(file, templateName, options) {
