...
import { logMessage } from '../../dist/esm/lib/utils/logger.js';
...
describe('Ticket System Test Suite', function () {
    let PersistObjectTemplate, collections, requires, TicketItemComment, TicketItemApproval, projectSemotus, Person, Project, db, client, securityPrincipal, cfg, mongoHost;
    ...
    before(async function() {
        PersistObjectTemplate = Persistor(ObjectTemplate, null, ObjectTemplate);
        collections = JSON.parse(fs.readFileSync(__dirname + '/model/schema.json'));
        PersistObjectTemplate.setSchema(collections);
        PersistObjectTemplate.setDB({}, PersistObjectTemplate.DB_Mongo);

        cfg = new nconf.Provider();
        cfg.argv().env();
        mongoHost = cfg.get('mongoHost') ? cfg.get('mongoHost') : 'localhost';
