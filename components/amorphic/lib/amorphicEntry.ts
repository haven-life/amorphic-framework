import {ContinuedSessionRet, ExpressSession, InitialSessionRet, isContinuedSession} from './types/AmorphicTypes';
...
export async function amorphicEntry(req: Request, res: Response, next: NextFunction) {
    ...
    if (!req.originalUrl.match(/amorphic\/init/)) {
        ...
        if (req.originalUrl.match(/([A-Za-z0-9_-]*)\.cached.js.map/)) {
            ...
            const amorphicSession: InitialSessionRet | ContinuedSessionRet = await establishServerSession(req, appName, 'initial', false, null);
            let time = process.hrtime();

            if (req.method === 'POST' && isContinuedSession(amorphicSession) && amorphicSession.objectTemplate.controller.processPost) { // processPost route so continued Server Session
